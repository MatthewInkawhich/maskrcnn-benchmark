...
class COCOeval:
    ...
    def summarize(self, xview=False, more_sizes=False):
        '''
        Compute and display summary metrics for evaluation results.
        Note this functin can *only* be applied on the default parameter setting
        '''
        def _summarize( ap=1, iouThr=None, areaRng='all', maxDets=100, print_class_wise=False):
            p = self.params
            ...
            if len(s[s>-1])==0:
                ...
                # ADDED: https://github.com/kimyoon-young/centerNet-deep-sort/blob/master/tools/cocoeval.py
                # Calculate per class AP
                if print_class_wise and ap == 1:
                    num_classes = s.shape[2]
                    for i in range(0, num_classes):
                        class_ap = np.mean(s[:,:,i,:])
                        print('category {} : {:.3f}'.format(i, class_ap))
                    print("\n")

            print(iStr.format(titleStr, typeStr, iouStr, areaRng, maxDets, mean_s))
            return mean_s
        def _summarizeDets(xview=False, more_sizes=False):
            if xview:
                stats = np.zeros((12,))
        ...
        self.stats = summarize(xview=xview, more_sizes=more_sizes)
